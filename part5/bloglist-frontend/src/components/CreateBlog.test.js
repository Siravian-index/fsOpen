...
test.only('<BlogForm /> updates parent state and calls onSubmit', () => {
  ...
  const component = render(<CreateBlog setBlogs={setBlogs} user={user} />)
  ...
  expect(setBlogs.mock.calls[0][0].title).toBe('Blog Title')
  expect(setBlogs.mock.calls[0][0].author).toBe('Author')
  expect(setBlogs.mock.calls[0][0].url).toBe('http://blog-title.com')
})
